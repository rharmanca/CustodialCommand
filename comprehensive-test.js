...
async function testCreateCustodialNote() {
  try {
    const noteData = {
      ...
      issue: 'Test Issue',
      priority: 'medium',
      description: 'Test custodial note created by automated test suite',
...
async function testFileServing() {
  try {
    // First create an inspection with an image
    const inspectionWithImage = await testFileUpload();
    if (!inspectionWithImage || !inspectionWithImage.images || inspectionWithImage.images.length === 0) {
      recordTest('File Serving', false, 'No image uploaded to test file serving');
      return null;
    }

    const imageUrl = inspectionWithImage.images[0];
...
async function testDatabaseConsistency() {
  try {
    ...
    if (notesValid && notes.data.length > 0) {
      ...
      const requiredFields = ['id', 'location', 'issue', 'priority', 'description'];
