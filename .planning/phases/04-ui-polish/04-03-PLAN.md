...
<task type="checkpoint:decision" gate="blocking">
  ...
  <files>.planning/phases/04-ui-polish/04-03-PLAN.md</files>
  <decision>Confirm tool, agent, and skill inventory for quick-capture layout simplification</decision>
  <context>
    Inventory useful tools, agents, and skills and orchestrate them before implementation. Recommended baseline for this plan:
    - frontend-design (camera-first progressive disclosure layout)
    - webapp-testing (mobile flow and capture-path verification)
    - web-design-guidelines (touch and accessibility checks)
    - tools: read/edit/apply_patch and bash verification commands
    - agents: gsd-plan-checker for plan-quality checks; explore/general if code paths diverge from assumptions
  </context>
  <options>
    <option id="option-a">
      <name>Use recommended baseline skills</name>
      <pros>Keeps work focused and fast.</pros>
      <cons>Less exploration of alternative UX patterns.</cons>
    </option>
    <option id="option-b">
      <name>Expand skill inventory first</name>
      <pros>More exhaustive UX consideration.</pros>
      <cons>Higher planning overhead.</cons>
    </option>
  </options>
  <action>Record chosen tools, agents, and skills and orchestrate them before changing capture-page information hierarchy.</action>
  <verify>Checkpoint decision is recorded and selected tool/agent/skill choices are documented before implementation.</verify>
