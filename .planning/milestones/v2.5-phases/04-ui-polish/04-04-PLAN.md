...
<task type="checkpoint:decision" gate="blocking">
  ...
  <files>.planning/phases/04-ui-polish/04-04-PLAN.md</files>
  <decision>Confirm tool, agent, and skill inventory for pending badge urgency behavior</decision>
  <context>
    Inventory useful tools, agents, and skills and orchestrate them before implementation. Recommended baseline for this plan:
    - frontend-design (badge hierarchy and urgency signaling)
    - react-best-practices (state updates and render behavior)
    - webapp-testing (real-time update and regression checks)
    - tools: read/edit/apply_patch and bash verification commands
    - agents: gsd-plan-checker for structured validation; explore/general if ownership of count logic is unclear
  </context>
  <options>
    <option id="option-a">
      <name>Use recommended baseline skills</name>
      <pros>Fast execution with practical safeguards.</pros>
      <cons>May miss niche animation nuance guidance.</cons>
    </option>
    <option id="option-b">
      <name>Expand skill inventory first</name>
      <pros>Broader review of animation and update patterns.</pros>
      <cons>Increases setup time before coding.</cons>
    </option>
  </options>
  <action>Record selected tools, agents, and skills and orchestrate them before implementing badge thresholds, pulse behavior, and update wiring.</action>
  <verify>Decision is recorded and selected tool/agent/skill choices are documented before code changes begin.</verify>
