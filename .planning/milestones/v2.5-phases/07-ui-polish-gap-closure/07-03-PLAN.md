...
must_haves:
  truths:
    - Camera appears first in Quick Capture visual hierarchy (DOM reorder, not CSS order)
    ...
    - Scroll direction state managed at App level
  artifacts:
    ...
    - path: src/App.tsx
      provides: Scroll direction state management
    - path: src/components/ui/FloatingActionButton.tsx
      provides: Visibility controlled by scroll direction prop
  key_links:
    - from: QuickNoteInput component
      to: Quick Capture page
      via: Collapsed prop and toggle callback
    - from: App scroll listeners
      to: FloatingActionButton visibility
      via: scroll-direction state prop
---
...
**FAB Scroll Behavior:**
- Hide FAB when user scrolls down (content visible)
- Show FAB when user scrolls up (returning to top)
